...
function loadStreak() {
  ...
  document.getElementById('streak-count').textContent = data.streak?.current ?? 0;
}
...
function makePomodoroUI(mode, prefix) {
  ...
  function updateProgress(remaining, duration) {
    if (!duration) return;
    el('pomo-progress').style.width = ((duration - remaining) / duration * 100) + '%';
  }
  ...
  function renderHistory() {
    ...
    container.innerHTML = sessions.map(s => {
      ...
      const ratings  = (s.energyLevel != null || s.focusQuality != null)
